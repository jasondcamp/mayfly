...
async def _forward_to_tg(tg_arn, method, path, headers, body, query_params):
    ...
    if target_type in ("instance", "ip"):
        # mayfly patch: upstream supports lambda targets only; proxy
        # instance/ip targets over HTTP (Id = hostname or IP).
...
async def _proxy_http_target(target, tg, method, path, headers, body, query_params):
    """mayfly patch: forward a data-plane request to an instance/ip target.

    The target Id is used as the hostname (DNS name or IP); adds ALB-style
    X-Forwarded-* and X-Amzn-Trace-Id headers like a real ALB would.
    """
    import urllib.error
